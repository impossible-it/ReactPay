...
export const createOrder = async (amount) => {                              // BASE  BASE   BASE  BASE   
  try {
    const response = await axios.get(`${BASE_API_URL}/${amount}/currency/RUB/niche/auto,kot,sidor,rock`);
...
export const createOrderSPB = async (amount) => {                     // BASE  BASE   BASE  BASE   
  try {
    const response = await axios.get(`${BASE_API_SBP}/${amount}/currency/RUB/niche/auto,kot,sidor,rock/bank/sbp`);
    return response.data[0];
  } catch (error) {
...
export const DNcreateOrder = async (amount) => {                               // DN DN DN DN
  try {
    const response = await axios.get(`${DN_API_URL}/${amount}/currency/RUB/niche/auto,kot,sidor,rock`);
    return response.data[0];
  } catch (error) {
    console.error('Error creating payment request:', error);
    throw error;
...
export const DNcreateOrderSPB = async (amount) => {                     // DN DN DN DN
  try {
    const response = await axios.get(`${DN_API_SBP}/${amount}/currency/RUB/niche/auto,kot,sidor,rock/bank/sbp`);
    return response.data[0];
  } catch (error) {
    console.error('Error creating payment request for SPB:', error);
    throw error;
